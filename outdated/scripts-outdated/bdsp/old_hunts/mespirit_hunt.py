...
def encounter_mespirit(ser: serial.Serial, vid: cv2.VideoCapture,):
    ...
    while True:
            ...
            for _ in range(60):
                ...
                if (current_text == 'Mesprit appeared!'):
                    mes_e = True
                    print('Mesprit appeared!')
                    break
                time.sleep(0.1)
...
def get_out_of_cave(ser: serial.Serial):
    for x in range(70):
        # print(f'pressing A {x}')
        _press(ser, 'A')
        time.sleep(0.25)
    _press(ser, 'd', duration=0.3)
    _press(ser, 's', duration=3.5)
    time.sleep(4)

def get_back_to_route(ser: serial.Serial, vid: cv2.VideoCapture,):
...
def get_to_route(ser: serial.Serial):
    _press(ser, 'X', sleep_time=1)
    _press(ser, 'A', sleep_time=1.5)
    _press(ser, '1', count=9)
    _press(ser, '4', count=10)
    _press(ser, 'A', sleep_time=1.5)
    _press(ser, 'A', sleep_time=9)
...
def main() -> int:
    ...
    with serial.Serial(args.serial, 9600) as ser, _shh(ser):
        ...
        # get_out_of_cave(ser=ser)
        # get_to_route(ser=ser)
        # get_mespirit_on_route(ser=ser, vid=vid,)
