...
def handle_encounter(vid: cv2.VideoCapture, ser: serial.Serial):
    ...
    await_not_pixel(ser, vid, x=x_val, y=y_val, pixel=(255, 255, 255))
    log_frame = getframe(vid)
    print(f'Unown pixel gone') 
    t0 = time.time()

    await_pixel(ser, vid, x=x_val, y=y_val, pixel=(255, 255, 255))
    print('Go Breloom!')
    t1 = time.time()
    ...
    await_pixel(ser, vid, x=1111, y=672, pixel=(238, 127, 115))
    time.sleep(0.5)
    ...
    await_pixel(ser, vid, x=x_val, y=y_val, pixel=(7, 7, 7))
    await_not_pixel(ser, vid, x=x_val, y=y_val, pixel=(7, 7, 7))
    print('Exited battle')
