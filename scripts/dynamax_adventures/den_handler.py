...
class DenHandler:
    ...
    def select_starter(self):
        ...
        try:
            first_attack = int(
                get_text(frame=frame, top_left=Point(y=175, x=971), bottom_right=Point(y=207, x=1041), invert=True)
            )
        except Exception as _:
            first_attack = 0
        try:
            first_specattack = int(
                get_text(frame=frame, top_left=Point(y=138, x=1191), bottom_right=Point(y=171, x=1250), invert=True)
            )
        except Exception as _:
            first_specattack = 0
        ...
        try:
            second_attack = int(
                get_text(frame=frame, top_left=Point(y=361, x=971), bottom_right=Point(y=394, x=1043), invert=True)
            )
        except Exception as _:
            second_attack = 0
        try:
            second_specattack = int(
                get_text(frame=frame, top_left=Point(y=323, x=1190), bottom_right=Point(y=356, x=1250), invert=True)
            )
        except Exception as _:
            second_specattack = 0
        ...
        try:
            third_attack = int(
                get_text(frame=frame, top_left=Point(y=547, x=969), bottom_right=Point(y=582, x=1050), invert=True)
            )
        except Exception as _:
            third_attack = 0
        try:
            third_specattack = int(
                get_text(frame=frame, top_left=Point(y=508, x=1193), bottom_right=Point(y=543, x=1250), invert=True)
            )
        except Exception as _:
            third_specattack = 0
    ...
    def handle_choose_pokemon(self):
        ...
        while not any(value[0] == current_name for value in name_map.values()):
            ...
            time.sleep(3)
            frame = getframe(self.vid)
            current_name = get_text(
                frame=frame, top_left=Point(y=87, x=279), bottom_right=Point(y=127, x=595), invert=True
            )
    ...
    def handle_den_search(self, contains_shiny: bool, beat_legend: bool):
        ...
        if not streak_data["search_dens"]:
            streak_data["paths"] = []
            streak_data["swaps"] = []
            self.config.update(
                {
        ...
        if (
            streak_data["wins"] > 0
            and streak_data["battles"] < 2
            or streak_percent >= streak_data["win_ratio_threshold"]
        ):
            keep_dungeon = True
    ...
    def swap_if_needed(self):
        ...
        streak_data = self.config.get("streak_data")
        if len(streak_data["swaps"]) == 3 and streak_data["search_dens"]:
        ...
        try:
            curr_attack = int(
                get_text(frame=frame, top_left=Point(y=211, x=969), bottom_right=Point(y=247, x=1056), invert=True)
            )
        except Exception as _:
            curr_attack = 0
        try:
            curr_specattack = int(
                get_text(frame=frame, top_left=Point(y=178, x=1192), bottom_right=Point(y=210, x=1249), invert=True)
            )
        except Exception as _:
            curr_specattack = 0
        ...
        try:
            temp_attack = int(
                get_text(frame=frame, top_left=Point(y=398, x=971), bottom_right=Point(y=435, x=1049), invert=True)
            )
        except Exception as _:
            temp_attack = 0
        try:
            temp_specattack = int(
                get_text(frame=frame, top_left=Point(y=359, x=1192), bottom_right=Point(y=399, x=1253), invert=True)
            )
        except Exception as _:
            temp_specattack = 0
    ...
    def extract_text(self, x1, y1, x2, y2, sortByX: bool):
        ...
        for i in range(len(boxes["text"])):
            ...
            if len(text) > 1:  # Ignore very short words (likely noise)
                x, y, _, _ = (boxes["left"][i], boxes["top"][i], boxes["width"][i], boxes["height"][i])
    ...
    def handle_pathing(self):
        ...
        streak_data = self.config.get("streak_data")
        if len(streak_data["paths"]) == 3 and streak_data["search_dens"]:
