...
from services.common import Point, press, getframe, get_text, color_near
import cv2
...
class BattleHandler:
    ...
    def attack_with_move(self):
        ...
        moves_order = {"Super effective": 0, "Effective": 1, "Not very effective": 2}
        ...
        move_list = [
            Move(e=move1E, p=move1P, index=0),
            Move(e=move2E, p=move2P, index=1),
            Move(e=move3E, p=move3P, index=2),
            Move(e=move4E, p=move4P, index=3),
        ]
        ...
        press(self.ser, "s" if distance < 0 else "w", count=abs(distance), sleep_time=0.2)
        press(self.ser, "A", sleep_time=1.5)
        press(self.ser, "A", sleep_time=0.75)
    ...
    def dynamax_if_available(self):
        ...
        for _ in range(10):
            if color_near(frame[590][728], (79, 0, 222)):
