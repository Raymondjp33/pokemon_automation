...
def main() -> int:
    ...
    with serial.Serial(args.serial, 9600) as ser, _shh(ser):
        ...
        while dex_num < 1025:
            ...
            has_pokemon = check_if_pokemon(vid)
            print(f'{dex_num}: {has_pokemon}')
            ...
            new_pokemon_pos = get_box_location(dex_num, False)

            # Go to next pokemon
            make_move(ser, curr_pos=old_pokemon_pos[2], move_pos=new_pokemon_pos[2], move_vertical=False)
            make_move(ser, curr_pos=old_pokemon_pos[1], move_pos=new_pokemon_pos[1], move_vertical=True)
