...
def main() -> int:
    ...
    config = ConfigManager(file_path=Path(__file__).resolve().parent / 'config.json')
    ...
    missing = {}
    for x in range(493):
        if not owned_pokemon.__contains__(x) and not owned_shiny_pokemon.__contains__(x):
            missing[x] = (get_key_from_value(pokemon_map, x))

    print(missing.__len__())
    ...
    with serial.Serial(args.serial, 9600) as ser, _shh(ser):
        ...
        pokemon_to_get = config.get('pokemon_to_get')
        
        from_box = get_box_location(config.get('from_box'), False)
        from_pokemon_num = config.get('from_num')

        for num in pokemon_to_get:
            pokemon_boxed = owned_pokemon.__contains__(num) or owned_shiny_pokemon.__contains__(num)

            if not pokemon_boxed:
                continue

            pokemon_is_shiny = not owned_pokemon.__contains__(num)

            from_pokemon = get_box_location(from_pokemon_num, False)
            to_pokemon = get_box_location(num, pokemon_is_shiny)
            ...
            # Remove pokemon from boxed pokemon
            if  pokemon_is_shiny:
                owned_shiny_pokemon.remove(num)
                with open("boxed_shiny_pokemon.json", "w") as f:
                    json.dump(sorted(list(owned_shiny_pokemon)), f)
            else:
                owned_pokemon.remove(num)
                with open("boxed_pokemon.json", "w") as f:
                    json.dump(sorted(list(owned_pokemon)), f)

            pokemon_to_get.remove(num)

            from_pokemon_num += 1
            config.update({'from_num': from_pokemon_num, "pokemon_to_get": pokemon_to_get})
            from_pokemon = get_box_location(from_pokemon_num, False)
            ...
            if (from_pokemon_num > 30):
                from_pokemon_num = 1
                _press(ser, 'R', sleep_time = 1.5)
                from_box = from_box = get_box_location(config.get('from_box') + 1, False)
                config.update({'from_num': from_pokemon_num, 'from_box': config.get('from_box') + 1})
