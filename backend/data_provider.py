...
def emit_pokemon_data():
    ...
    cursor.execute(
        ...
        (switch1_hunt_id, switch2_hunt_id, switch3_hunt_id),
    )
        ...
        pokemon_data.append(
            {
                ...
                "catches": [
                    ...
                    for _, _, ts, enc, method, switchUsed, catch_name, tdens, _ in catch_rows
                ],
            }
        )

    print("[WebSocket] Emitting pokemon data")
...
def watch_file():
    ...
    while True:
        try:
            ...
            print("[WebSocket] Emitting update")
...
def get_pokemon_stats(cursor):
    ...
    totalEggs = cursor.execute(
        "SELECT SUM(encounters) FROM hunt_encounters WHERE encounter_method = ?", ("egg",)
    ).fetchone()[0]
    averageEggs = totalEggs / totalEggShinies

    totalStaticShinies = cursor.execute(
        "SELECT COUNT(*) FROM catches WHERE encounter_method = ?", ("static",)
    ).fetchone()[0]
    totalStaticFails = cursor.execute("SELECT COUNT(*) FROM fails WHERE encounter_method = ?", ("static",)).fetchone()[
        0
    ]
    totalStatic = cursor.execute(
        "SELECT SUM(encounters) FROM hunt_encounters WHERE encounter_method = ?", ("static",)
    ).fetchone()[0]
    averageStatic = totalStatic / max(totalStaticShinies + totalStaticFails, 1)

    totalWildShinies = cursor.execute("SELECT COUNT(*) FROM catches WHERE encounter_method = ?", ("wild",)).fetchone()[
        0
    ]
    totalWildFails = cursor.execute("SELECT COUNT(*) FROM fails WHERE encounter_method = ?", ("wild",)).fetchone()[0]
    totalWild = cursor.execute(
        "SELECT SUM(encounters) FROM hunt_encounters WHERE encounter_method = ?", ("wild",)
    ).fetchone()[0]
    averageWild = totalWild / max(totalWildShinies + totalWildFails, 1)

    totalOldWildShinies = cursor.execute(
        "SELECT COUNT(*) FROM catches WHERE encounter_method = ?", ("oldwild",)
    ).fetchone()[0]
    totalOldWildFails = cursor.execute(
        "SELECT COUNT(*) FROM fails WHERE encounter_method = ?", ("oldwild",)
    ).fetchone()[0]
    totalOldWild = cursor.execute(
        "SELECT SUM(encounters) FROM hunt_encounters WHERE encounter_method = ?", ("oldwild",)
    ).fetchone()[0]
    averageOldWild = totalOldWild / max(totalOldWildShinies + totalOldWildFails, 1)

    totalDAShinies = cursor.execute(
        "SELECT COUNT(*) FROM catches WHERE encounters IS NOT NULL AND encounter_method = ?",
        ("da",),
    ).fetchone()[0]
    ...
    return {
        ...
        "totalStaticShinies": totalStaticShinies,
        "totalStatic": totalStatic,
        "averageStatic": averageStatic,
        "totalWildShinies": totalWildShinies,
        "totalWild": totalWild,
        "averageWild": averageWild,
